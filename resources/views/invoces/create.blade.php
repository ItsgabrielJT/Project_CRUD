    ...
    <div class="py-12">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                ...
                <form class="grid gap-8 grid-cols-1" @if($invoce->id)
                    action="{{ route('invoce.update', ["invoce" => $invoce->id ]) }}" @else
                    action="{{ route('invoce.store') }}" @endif enctype="multipart/form-data" method="POST">
                    @if($invoce->id) {{ method_field("PUT") }} @endif
                                ...
                                <div class="col-span-3 sm:col-span-2">
                                    ...
                                    <div class="mt-1 flex rounded-md shadow-sm">
                                        <select name="serie" class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-md sm:text-sm border-gray-300">
                                            ...
                                            <option value="B">Gama Media</option>
                                            <option value="C">Gama Baja</option>
                                        </select>
                                    </div>
                                    @error('serie')
                                    <span class=" text-sm text-red-600" role="alert">
